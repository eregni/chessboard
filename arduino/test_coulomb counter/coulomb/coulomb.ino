...
#define INT_BATTERY_RPI 7

#define LED 13 // Standard Arduino LED


#define ADDRESS_BATSTATE 0  //address (first byte) in EEPROM for battery state

// Change the following two lines to match your battery
// and its initial state-of-charge:

volatile double battery_mAh = 2000.0; // milliamp-hours (mAh)
// volatile double battery_mAh = readBatState();
volatile double battery_percent = 54.7;  // state-of-charge (percent)

//Send a interrupt pulse to rpi every change of 0.1% battery mAh (Don't give the poor rpi soo many interrupts :-)
double battery_message_trigger = 0;
const double battery_message_limit = battery_mAh * 0.001;
...
void setup()
{
  ...
  pinMode(INT_BATTERY_RPI, OUTPUT);
  pinMode(INT_BATTERY_RPI, LOW);
...
void myISR() // Run automatically for falling edge on D3 (INT1)
{
  ...
  {
    battery_message_trigger += ah_quanta;
  ...
  // send interrupt pulse to rpi if necessary
  if (abs(battery_message_trigger) >= battery_message_limit){
    digitalWrite(INT_BATTERY_RPI, HIGH);
    digitalWrite(INT_BATTERY_RPI, LOW);
    battery_message_trigger = 0;
  }
}
...
void writeBatState(){
  ...
  for (int i = 0; i < 4; i++){
    EEPROM.write(ADDRESS_BATSTATE + i, data.b[i]);
  }
}
...
float readBatState(){
  ...
  for (int i = 0; i < 4; i++){
    data.b[i] = EEPROM.read(ADDRESS_BATSTATE + i);
  }
  return data.f;
}